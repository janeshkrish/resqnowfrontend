...
export type DailySettlementRow = {
  ...
  total_technician_earnings: number;
...
// ---- Phase 3: Payout Batches ----
export async function getPayoutBatches(limit = 50) {
  const { data } = await adminApi.get<{ batches: PayoutBatchRow[] }>(
...
export async function createPayoutBatch(paymentIds: number[]) {
  const { data } = await adminApi.post<{ batchId: number, totalAmount: number, count: number }>(
    "/finance/payout-batches",
    { paymentIds }
  );
  return data;
...
export async function completePayoutBatch(batchId: number) {
  const { data } = await adminApi.put<{ batchId: number, status: string }>(
    `/finance/payout-batches/${batchId}/complete`
  );
  return data;
}

// ---- Phase 4: Daily Settlement Reports ----
export async function getSettlementReports(limit = 30) {
  const { data } = await adminApi.get<{ settlements: DailySettlementRow[] }>(
...
export async function generateSettlementReport(date?: string) {
  const { data } = await adminApi.post<DailySettlementRow>(
    "/finance/settlement-report/generate",
    { date }
  );
  return data;
}

// ---- Phase 5: Fraud Flag Updates ----
export async function updateFraudFlagStatus(flagId: number, status: string) {
  const { data } = await adminApi.put<{ success: boolean }>(
    `/finance/fraud-flags/${flagId}`,
    { status }
  );
  return data;
}
