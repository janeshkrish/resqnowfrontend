...
const EVServiceRequest = () => {
  ...
  return (
    <div className={cn("min-h-screen bg-gradient-to-br from-green-50 to-emerald-50", isMobile ? "py-3 pb-safe" : "py-4 md:py-8 pb-24 md:pb-8")}>
      <div className={cn("container mx-auto max-w-4xl", isMobile ? "px-3" : "px-3 md:px-4")}>
        ...
        <div className={cn("bg-white border border-green-100", isMobile ? "rounded-2xl shadow-lg p-4" : "rounded-3xl shadow-2xl p-4 md:p-12")}>
          <div className={cn(isMobile ? "text-left mb-5" : "text-center mb-8")}>
            <div className={cn("inline-flex items-center justify-center p-3 bg-green-100 rounded-full", isMobile ? "mb-3" : "mb-4")}>
              <Zap className={cn(isMobile ? "h-6 w-6" : "h-8 w-8", "text-green-600")} />
            </div>
          ...
          <>
            ...
            <div className={cn("mt-8", isMobile ? "sticky bottom-3 z-20 bg-white/95 backdrop-blur rounded-xl border border-green-100 p-3 shadow-lg space-y-2" : "flex gap-3")}>
              {currentStep > 1 && (
              ...
              <Button
                ...
                className={cn("flex-1 py-6 text-base font-semibold bg-green-600 hover:bg-green-700 text-white", isMobile && "w-full py-3")}
              >
                {isSubmitting ? (
                  <>
