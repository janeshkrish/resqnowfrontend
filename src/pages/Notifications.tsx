...
const Notifications = () => {
    ...
    return (
        <div className="min-h-screen bg-muted relative pb-24">
            {/* Sticky Header */}
            <div className="sticky top-0 z-40 bg-card dark:bg-slate-900/80 backdrop-blur-xl border-b border-border px-4 h-16 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <Button variant="ghost" size="icon" onClick={() => navigate(-1)} className="rounded-full hover:bg-muted/50 -ml-2">
                        <ArrowLeft className="w-5 h-5 text-muted-foreground" />
                    </Button>
                    <h1 className="text-xl font-black text-foreground tracking-tight">Updates</h1>
                </div>
                <Button variant="ghost" size="icon" className="rounded-full text-muted-foreground/80 hover:text-foreground">
            ...
            <div className="pt-2">
                {MOCK_NOTIFICATIONS.length > 0 ? (
                    <div className="bg-card dark:bg-slate-900 border-y border-border divide-y divide-slate-100">
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start gap-2 mb-1">
                                        <h3 className={`font-bold text-sm truncate ${notif.unread ? "text-foreground" : "text-muted-foreground"}`}>
                                            {notif.title}
                                        </h3>
                                    ...
                                    <p className={`text-sm leading-snug line-clamp-2 ${notif.unread ? "text-muted-foreground font-medium" : "text-muted-foreground/80"}`}>
                                        {notif.message}
                                    </p>
                                </div>
                    ...
                    <div className="flex flex-col items-center justify-center pt-32 px-6 text-center">
                        ...
                        <p className="text-muted-foreground/80 text-sm">We'll notify you when there's something new.</p>
                    </div>
                )}
            </div>
        </div>
